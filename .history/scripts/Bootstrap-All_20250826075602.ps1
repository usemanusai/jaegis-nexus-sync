...
function Ensure-Repo($owner,$name,$token){
  ...
  if (-not $exists) {
    ...
    if (Test-Cmd 'gh') {
      ...
      $headers = @{ Authorization = "token $token"; 'User-Agent' = 'NexusSync-Bootstrap' }
      $body = @{ name = $name; @namespace = $owner; private = $false; auto_init = $true } | ConvertTo-Json
      try { Invoke-RestMethod -Method POST -Uri "https://api.github.com/user/repos" -Headers $headers -Body $body } catch { Write-Err "Failed to create repo via API: $($_.Exception.Message)"; exit 1 }
    }
  }
}
...
if ($EnableTLS) {
  ...
  if (Test-Path $cfgPath) {
    # Patch config for TLS
    $y = Get-Content $cfgPath -Raw
    $y = $y -replace 'tls: false','tls: true'
    if ($Domain) { $y = $y -replace 'domain: ""',(\"domain: '" + ($Domain -replace "'","''") + "'\") }
    if ($Email)  { $y = $y -replace 'email: ""',(\"email: '" + ($Email -replace "'","''") + "'\") }
