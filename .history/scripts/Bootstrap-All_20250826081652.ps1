...
if ($needsInstall -and (Test-Cmd 'winget')) {
  Write-Info "Some tooling missing (node:$needNode git:$needGit gh:$needGh) — attempting install via winget"
  try { Ensure-Admin } catch { Write-Warn "Elevation may be required for installs; continuing best-effort." }
  Ensure-Tooling
} else {
