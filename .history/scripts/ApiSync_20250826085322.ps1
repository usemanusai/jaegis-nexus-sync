...
function Ensure-Branch($owner,$repo,$branch){
  ...
  $refUrl = "$repoUrl/git/ref/heads/$branch"
  try { Invoke-RestMethod -Method GET -Uri $refUrl -Headers $headers | Out-Null; return }
    ...
    try {
      $sha = (Invoke-RestMethod -Method GET -Uri "$repoUrl/git/ref/heads/$default" -Headers $headers).object.sha
...
function Put-Content($owner,$repo,$branch,$full,$root){
  ...
  $rel = $rel.Substring((Resolve-Path $root).Path.Length).TrimStart('\\','/')
