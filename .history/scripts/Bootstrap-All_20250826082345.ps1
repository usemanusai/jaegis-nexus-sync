...
function Winget-Install($id, $name){
  Write-Info "Installing $name ($id) via winget (user scope first)..."
  try { winget install --id $id --silent --accept-source-agreements --accept-package-agreements -e --scope user | Out-Null } catch {}
  try { winget install --id $id --silent --accept-source-agreements --accept-package-agreements -e | Out-Null } catch {}
}
...
function Ensure-Tooling(){
  ...
  if ($ForceInstallGh) {
    Write-Info "Forcing reinstall of GitHub CLI (user scope)"
    try { winget uninstall --id GitHub.cli --silent -e | Out-Null } catch {}
    try { winget install --id GitHub.cli --silent --accept-source-agreements --accept-package-agreements -e --scope user | Out-Null } catch {}
  }

  # Fallback: portable GitHub CLI if still missing after winget attempts
  if (-not (Test-Cmd 'gh')) {
    Write-Info "Installing GitHub CLI (portable)"
    try {
      $destRoot = Join-Path $env:LOCALAPPDATA 'Jaegis\\bin'
      New-Item -ItemType Directory -Force -Path $destRoot | Out-Null
      $zipPath = Join-Path $env:TEMP 'gh_portable.zip'
      $url = 'https://github.com/cli/cli/releases/latest/download/gh_2.57.0_windows_amd64.zip'
      ...
      $ghExe = Get-ChildItem -Path $extractDir -Recurse -Filter 'gh.exe' | Select-Object -First 1
      if ($ghExe -and (Test-Path $ghExe.FullName)) {
        Copy-Item $ghExe.FullName (Join-Path $destRoot 'gh.exe') -Force
        Ensure-Path $destRoot
        [System.Environment]::SetEnvironmentVariable('Path', "$env:Path;$destRoot", 'User')
        Write-Info "GitHub CLI installed to $destRoot"
      } else { Write-Warn "Portable gh.exe not found after extraction" }
    } catch { Write-Warn "Portable GitHub CLI install failed: $($_.Exception.Message)" }
  }
...
$needNode = -not (Test-Cmd 'node')
$needGit  = -not (Test-Cmd 'git')
$needGh   = -not (Test-Cmd 'gh')
$needsInstall = ($needNode -or $needGit -or $needGh)

if ($needsInstall -and (Test-Cmd 'winget')) {
  Write-Info "Some tooling missing (node:$needNode git:$needGit gh:$needGh) - installing via winget (user scope preferred, no elevation)"
  Ensure-Tooling
} else {
  Write-Info "Skipping tool installation (requirements satisfied or winget not available)."
}
