...
async function processIngest(job: Job) {
  ...
  await prisma.embedding.create({
    data: {
      documentId,
      vector,
      vectorPg: undefined, // will be populated via updateRaw below for pgvector column
      model,
      dim: vector.length,
    },
  });
  // Update pgvector column via raw SQL
  await prisma.$executeRawUnsafe(
    `UPDATE "Embedding" SET "vectorPg" = $1::vector WHERE "documentId" = $2 ORDER BY "createdAt" DESC LIMIT 1`,
    `[${vector.join(',')}]`,
    documentId
  );
}
