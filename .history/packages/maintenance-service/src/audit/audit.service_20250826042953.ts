...
import { enqueueResearch } from '../clients/research.client'
import { ingestDoc } from '../clients/rag.client'
import { NarrativeService } from '../narrative/narrative.service'

@Injectable()
export class AuditService {
  private _status = { lastRun: null as null | string, lastPR: null as null | { url: string, number: number } }
  constructor(private readonly prisma: PrismaService, private readonly redis: RedisService, private readonly osv: OsvService, private readonly narratives: NarrativeService) {}
  ...
  async runAuditNow() {
    ...
    for (const m of manifests) {
      ...
      if (bumpChanges.length) {
        try {
          const pr = await openPR(bumpChanges, { branch: 'chore/auto-patch-bumps', title: 'chore: auto patch bumps', message: 'Automated safe patch updates' })
          this._status.lastPR = pr && pr.url ? { url: pr.url, number: pr.number } : null
        } catch {}
      }
    }
  ...
  private async storeVulnerability(ecosystem: string, name: string, v: any) {
    ...
    if (url) { try { await enqueueResearch(url) } catch {} }
    if (v.summary) {
      const narrative = await this.narratives.advisoryNarrative({ name: dep.name, ecosystem: dep.ecosystem, id: v.id, summary: v.summary, url })
      await ingestDoc(`advisory://${dep.ecosystem}/${dep.name}/${v.id}`, narrative)
    }
  }

  private buildNarrative(name: string, ecosystem: string, id: string, summary: string, url?: string|null) {
    return `# Advisory Narrative\n\n- Package: ${name} [${ecosystem}]\n- Advisory: ${id}\n- Source: ${url||'n/a'}\n\n## Summary\n${summary}\n\n## Reasoning\nThis upgrade is prioritized due to security considerations. Patch-level updates are chosen to avoid breaking changes while mitigating risks.`
  }
