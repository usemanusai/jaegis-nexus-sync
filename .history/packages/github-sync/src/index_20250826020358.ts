...
export async function syncRepo(owner: string, repo: string, token?: string) {
  ...
  await fetch(RAG_INGEST, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source: `github://${owner}/${repo}/__blueprint.md`, content: blueprint }) })
  // Send files
  for (const f of files) {
    const body = {
