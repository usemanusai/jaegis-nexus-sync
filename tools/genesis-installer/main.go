...
func writeComposeOverride(root string, opts ComposeOpts, force bool) error {
	...
	content := `version: "3.9"
	fileCmd := "      - --providers.file.directory=/etc/traefik/dynamic\n      - --providers.file.watch=true\n"
...
` + tlsPorts + `    depends_on:
      ...
      - maintenance
    volumes:
` + vol + `      - ./traefik/dynamic:/etc/traefik/dynamic
`
...
func quickInstall(ctx context.Context, root string, cfg *Config) error {
	...
	opts := ComposeOpts{ TLS: false }
	if cfg != nil { opts.TLS = cfg.Proxy.TLS; opts.Domain = cfg.Proxy.Domain; opts.Email = cfg.Proxy.Email }
	if err := writeComposeOverride(root, opts, false); err != nil { return err }
	...
	// HTML report
	h := strings.ReplaceAll(string([]byte(`<!doctype html><html><body>placeholder</body></html>`)), "placeholder", "See report.txt for details")
	if b, err := os.ReadFile(filepath.Join(root, "tools", "genesis-installer", "report.html.tmpl")); err == nil {
		h = string(b)
		h = strings.ReplaceAll(h, "{{.Now}}", time.Now().Format(time.RFC3339))
		h = strings.ReplaceAll(h, "{{.JWT}}", jwtSecret)
		h = strings.ReplaceAll(h, "{{.RAG}}", ragApiKey)
	}
	_ = os.WriteFile(filepath.Join(root, "install-report", "report.html"), []byte(h), 0644)
...
func main(){
	...
	switch mode {
	case Quick:
		if err := quickInstall(ctx, root, cfg); err != nil { fmt.Println("Error:", err); os.Exit(1) }
	case Interactive:
		if err := interactiveInstall(ctx, root); err != nil { fmt.Println("Error:", err); os.Exit(1) }
	case Headless:
		if err := headlessInstall(ctx, root, *cfgPath); err != nil { fmt.Println("Error:", err); os.Exit(1) }
	}
